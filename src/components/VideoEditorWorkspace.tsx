...
export function VideoEditorWorkspace({ nodeId, onClose }: VideoEditorWorkspaceProps) {
  ...
  // Clean up any old video editor node cards from canvas and re-route edges to 'global'
  useEffect(() => {
    const veNodes = canvasEngine.getNodes().filter((n) => n.type === 'util.videoEditor');
    veNodes.forEach((n) => {
      const edges = canvasEngine.getEdges().filter((e) => e.target === n.id);
      edges.forEach((e) => {
        canvasEngine.addEdge({
          ...e,
          id: `e_global_${Date.now()}_${Math.random().toString(36).substr(2, 4)}`,
          target: 'global',
        });
      });
      canvasEngine.removeNode(n.id);
    });
  }, []);
